...
const UnapprovedCashSales = () => {
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                const { data } = await mtaApi.cash_sales.list_cash_sale('2');
                if (data.status === 200) {
    ...
    const approve = async () => {
        try {
            const { data } = await mtaApi.cash_sales.approve_cash_sale(selectedRowData.id)
            if (data.status === 200) {
                navigate("/sales/approved-cash-sales-receipts");
            }
        } catch (error) {
