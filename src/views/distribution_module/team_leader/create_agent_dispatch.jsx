...
const CreateDispatchToAgents = () => {
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                ...
                const { data } = await mtaApi.team_leader_dispatch.list_stock_available(params)
                if (data.status === 200) {
    ...
    const onSubmit = async (data) => {
        const formattedData = {
            ...data,
            mobilephone_teamleaderstock_id: selectedRows.length > 0 ? selectedRows[0].id : null,
        };
        try {
            
            const response = await mtaApi.team_leader_dispatch.create_agent_dispatch(formattedData)
            if (response.data.status === 200) {
                navigate("/distribution/team-leader-approve-dispatch-to-agents")
                reset()
            } else {
    ...
    return (
        <div>
            {currentDiv === "listpage" && (
                <div>
                    <PageHeader currentpage="Create Dispatch to Agents" href="/inventory/distribution-dashboard" activepage="Dispatch" mainpage="Select Device to Send To Agents" />
            ...
            {currentDiv === "details" && (
                <div>
                    <PageHeader currentpage="Create Dispatch to Agents" href="/inventory/distribution-dashboard" activepage="Dispatch" mainpage="Device Details" />
                                ...
                                <div className="box-body p-0">
                                    ...
                                    <div id="profile-settings-1" role="tabpanel" aria-labelledby="profile-settings-item-1">
                                        ...
                                        <div className="box border-0 shadow-none mb-0">
                                            ...
                                            <div className="box-header">
                                                <h5 className="box-title  text-center">Enter Agent Dispatch Details</h5>
                                            </div>
                                            <div className="box-body">
                                                <div>
                                                    <div className="grid lg:grid-cols-2 gap-6">
                                                    ...
                                                    <div className="space-y-2">
                                                            <label className="ti-form-label mb-0">Select Agent</label>
                                                            <select {...register("agent_id", { required: true })} className="my-auto ti-form-input focus:outline-none focus:ring-2 focus:ring-blue-500">
                                                                <option value="">Select agent ...</option>
                                                                {teamleaders.map(user => (
                                                                    <option key={user.id} value={user.id}>{user.name}</option>
                                                                ))}
                                                            </select>
                                                            {errors.agent_id && <span className="text-red-500">{errors.agent_id.message}</span>}
                                                        </div>
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            <input type="date" {...register("teamleader_dispatch_date", { required: true })} className="my-auto ti-form-input" placeholder=" ... Enter dispatch date" required />
                                                        </div>

                                                        <div className="space-y-2">
                                                            <label className="ti-form-label mb-0">Product Remarks</label>
                                                            <input type="text" {...register("teamleader_remarks", { required: true })} className="ti-form-input" placeholder=" ...Enter product remarks" required />
                                                        </div>
...
export default CreateDispatchToAgents;
