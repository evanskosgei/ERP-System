...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
  ...
  useEffect(() => {
    if (local_varaiable.dataNavLayout == 'horizontal' && window.innerWidth >= 992) {
      clearMenuActive()
    }
  }, [])
            ...
            if (items.children) {
              items.children.filter((submenu) => {
                ...
                if (dd === submenu.path + '/') {
                  ...
                  items.active =
                    theme.dataNavLayout == 'horizontal' || theme.dataNavStyle == 'icon-hover' ? false : true
                  items.selected = true
                  submenu.active = true
        ...
        if (!(local_varaiable.dataVerticalStyle.dir === 'rtl')) {
          if (Math.abs(check) > Math.abs(marginLeftValue)) {
            ...
            menuNav.style.marginLeft =
              Number(menuNav.style.marginLeft.split('px')[0]) - Math.abs(mainContainer1Width) + 'px'
          ...
          if (Math.abs(check) > Math.abs(marginRightValue)) {
            ...
            menuNav.style.marginRight =
              Number(menuNav.style.marginRight.split('px')[0]) - Math.abs(mainContainer1Width) + 'px'
  ...
  return (
    <Fragment>
      <div></div>
      <aside className='app-sidebar' id='sidebar' onMouseOver={() => Onhover()} onMouseOut={() => Outhover()}>
            ...
            <ul className='main-menu'>
              {menuitems.map((firstlayer) => (
                <React.Fragment key={Math.random()}>
                  {firstlayer.menutitle ? <li className='slide__category'>{firstlayer.menutitle}</li> : ''}
                  {firstlayer.Items.map((secondlayer) =>
                    secondlayer.type === 'link' ? (
                      <li className={`slide ${secondlayer.active ? 'active' : ''}`} key={Math.random()}>
                        ...
                        >
                          {secondlayer.icon} <span className='side-menu__label'>{secondlayer.title}</span>
                        </Link>
                      </li>
                          ...
                          {secondlayer.children.map((thirdlayer) => (
                            <React.Fragment key={Math.random()}>
                              {thirdlayer.type === 'link' ? (
                                ...
                                <li className={`slide has-sub ${secondlayer.active ? 'open' : ''}`}>
                                  <a
                                    href='#'
                                  ...
                                  >
                                    {thirdlayer.icon} <span className=''>{thirdlayer.title}</span>
                                    <i className='ri ri-arrow-right-s-line side-menu__angle'></i>
                                  </a>
                                        ...
                                        {fourthlayer.type === 'link' ? (
                                          <li className={`slide ${fourthlayer.active ? 'active' : ''}`}>
                                            <Link
                                              ...
                                              className={`side-menu__item ${fourthlayer.selected ? 'active' : ''}`}
                                            >
                                              {fourthlayer.icon}
                                              {fourthlayer.title}
                                            </Link>
                                              ...
                                              {fourthlayer.children.map((fivthlayer) => (
                                                <React.Fragment key={Math.random()}>
                                                  {fivthlayer.type === 'link' ? (
                                                    ...
                                                    >
                                                      {fivthlayer.icon} <span className=''>{fivthlayer.title}</span>
                                                      <i className='ri ri-arrow-right-s-line side-menu__angle'></i>
                                                    </a>
                                                  )}
                                                </React.Fragment>
