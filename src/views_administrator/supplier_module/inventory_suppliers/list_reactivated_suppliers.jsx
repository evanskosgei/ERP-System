...
const ReactivatedSuppliers = () => {
  ...
  const onGridReady = useEffect(() => {
    const ActiveSuppliers = async () => {
      try {
        const { data } = await mtaApi.suppliers.get_suppliers('5');
        if (data.status === 200) {
  ...
  return (
    <div>
      {currentDiv === "table" && (
        <div>
          <PageHeader currentpage="Reactivated Supplier" href="/supplier/dashboard/" activepage="Supplier" mainpage="Reactivated Supplier" />
          <div style={{ display: 'flex', alignItems: 'center', margin: '2' }}>
            ...
            />
            <CSVLink data={filteredData.length > 0 ? filteredData : rowData.length > 0 ? rowData : []} filename="Reactivated_suppliers.csv" separator={","} className="h-6 w-6 items-center mb-7 ml-7 mr-8 text-blue-600">
      ...
      {currentDiv === "Activesupplier" && (
        <div>
          <PageHeader currentpage="Reactivated Supplier Details" activepage="Supplier" mainpage="Reactivated Supplier Details" />
...
export default ReactivatedSuppliers;