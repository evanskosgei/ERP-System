...
const AgentsStockToReceive = () => {
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                ...
                const params = {"status":1, "stock_state":0}
                const { data } = await mtaApi.sales_agents.list_stock_toreceive(params)
                if (data.status === 200) {
    ...
    const onSubmit = async (data) => {
        const formattedData = {
            ...data,
            id: selectedRows.length > 0 ? selectedRows[0].id : null,
        };
        try {
            
            const response = await mtaApi.sales_agents.receive_dispatch(formattedData)
            if (response.data.status === 200) {
                navigate("/distribution/agent-stock-available")
                reset()
            } else {
    ...
    return (
        <div>
            {currentDiv === "listpage" && (
                <div>
                    <PageHeader currentpage="Sales Agents Receive Stock" href="/sales/dashboard" activepage="Sales" mainpage="Select Device to Receive from Team Leader" />
            ...
            {currentDiv === "details" && (
                <div>
                    <PageHeader currentpage="Sales Agents Receive Stock" href="/sales/dashboard" activepage="Sales" mainpage="Device Details" />
                                ...
                                <div className="box-body p-0">
                                    ...
                                    <div id="profile-settings-1" role="tabpanel" aria-labelledby="profile-settings-item-1">
                                                ...
                                                <div>
                                                    <div className="grid lg:grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-x-3">
                                                            ...
                                                            <span className="text-sm text-gray-800 dark:text-white/70">
                                                                {selectedRowData.distribution_center_name}
                                                            </span>
                                                        </div>

                                                        <div className="space-x-3">
                                                            <span className="text-sm font-bold">Team Leader Name :</span>
                                                            <span className="text-sm text-gray-800 dark:text-white/70">
                                                                {selectedRowData.teamleader_user_name}
                                                            </span>
                                                        </div>

                                                        <div className="space-x-3">
                                                            <span className="text-sm font-bold">Team Leader Dispatch Date :</span>
                                                            <span className="text-sm text-gray-800 dark:text-white/70">
                                                                {selectedRowData.teamleader_dispatch_date}
                                                            </span>
                                                        </div>
                                        ...
                                        <div className="box border-0 shadow-none mb-0">
                                            ...
                                            <div className="box-header">
                                                <h5 className="box-title  text-center">Enter Agents Dispatch Details</h5>
                                            </div>
                                            <div className="box-body">
                                                <div>
                                                    <div className="grid lg:grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-y-2">
                                                            <label className="ti-form-label mb-0">Received Date</label>
                                                            <input type="date" {...register("agent_received_date", { required: true })} className="my-auto ti-form-input" placeholder=" ... Enter received date" required />
                                                        </div>

                                                        <div className="space-y-2">
                                                            <label className="ti-form-label mb-0">Agent Product Remarks</label>
                                                            <input type="text" {...register("agent_remarks", { required: true })} className="ti-form-input" placeholder=" ...Enter product remarks" required />
                                                        </div>
...
export default AgentsStockToReceive;
