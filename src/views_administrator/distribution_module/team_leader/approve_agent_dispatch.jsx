...
const ApproveAgentsDispatch = () => {
    ...
    const columnDefs = [
        ...
        { headerName: "Date Dispatched", field: "teamleader_dispatch_date", sortable: true, editable: false, filter: true, flex: 2, resizable: true, minWidth: 10 },
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                const { data } = await mtaApi.team_leader_dispatch.list_agent_dispatch('2')
                console.log(data)
                if (data.status === 200) {
    ...
    const approve = async () => {
        try {
            const { data } = await mtaApi.team_leader_dispatch.approve_agent_dispatch(selectedRowData.id)
            if (data.status === 200) {
                navigate("/distribution/team-leader-list-dispatch-to-agents")
            }
        } catch (error) {
            const message = error.response?.data?.error ?? error.message;
            setAlert({ type: "error", message });
    ...
    return (
        <div>
            {currentDiv === "listpage" && (
                <div>
                    <PageHeader currentpage="Approve Dispatch to Agents" href="/inventory/distribution-dashboard/" activepage="Dispatch" mainpage="Stock Dispatched Pending Approval" />
                ...
                <div>
					...
					<div className= "box">
						<div className= "box-body p-0">
                            ...
                            <h5 className="box-title  text-center">Agent Stock Dispatch Details</h5>
                        </div>
									<div className= "box-body">
                                        ...
                                        <div className= "space-x-3">
                                            ...
                                            <span className= "text-sm text-gray-800 dark:text-white/70">{selectedRowData.teamleader_remarks}</span>
                                        </div>

                                        <div className= "space-x-3">
                                            <span className= "text-sm font-bold">Agents Name :</span>
                                            <span className= "text-sm text-gray-800 dark:text-white/70">{selectedRowData.agent_name}</span>
                                        </div>
									</div>
									</div>
									</div>
...
export default ApproveAgentsDispatch;
