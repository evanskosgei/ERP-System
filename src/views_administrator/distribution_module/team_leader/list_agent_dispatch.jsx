...
const ListTeamleaderDispatchToAgents = () => {
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                const { data } = await mtaApi.team_leader_dispatch.list_agent_dispatch('1')
                console.log(data)
                if (data.status === 200) {
    ...
    return (
        <div>
            {currentDiv === "listpage" && (
                <div>
                    <PageHeader currentpage="Stock Dispatched to Agents" href="/inventory/distribution-dashboard/" activepage="Distribution" mainpage="Stock sent to agents" />
...
export default ListTeamleaderDispatchToAgents;
