...
const TeamLeaderStockToReceive = () => {
    ...
    const onGridReady = useCallback(() => {
        const newUnApproved = async () => {
            try {
                ...
                const { data } = await mtaApi.team_leader_dispatch.list_stock_toreceive(params)
                if (data.status === 200) {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const response = await mtaApi.team_leader_dispatch.receive_dispatch(formattedData)
            if (response.data.status === 200) {
                navigate("/distribution/team-leader-stock-available")
                reset()
            } else {
    ...
    return (
        <div>
            {currentDiv === "listpage" && (
                <div>
                    <PageHeader currentpage="Team Leader Receive Stock" href="/inventory/distribution-dashboard" activepage="Distribution" mainpage="Select Device to Receive from Manager" />
            ...
            {currentDiv === "details" && (
                <div>
                    <PageHeader currentpage="Team Leader Receive Stock" href="/inventory/distribution-dashboard/" activepage="Distribution" mainpage="Device Details" />
                                            ...
                                            <div className="box-body">
                                                <div>
                                                    <div className="grid lg:grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-x-3">
                                                            <span className="text-sm font-bold">Stockist Dispatch Date :</span>
                                                            <span className="text-sm text-gray-800 dark:text-white/70">
                                                ...
                                                <div>
                                                    <div className="grid lg:grid-cols-2 gap-6">
                                                        ...
                                                        <div className="space-y-2">
                                                            ...
                                                            <input type="date" {...register("teamleader_received_date", { required: true })} className="my-auto ti-form-input" placeholder=" ... Enter received date" required />
                                                        </div>

                                                        <div className="space-y-2">
                                                            <label className="ti-form-label mb-0">Team Leader Product Remarks</label>
...
export default TeamLeaderStockToReceive;
